...
static struct att_char128_desc custs1_adxl_val_char         = {ATT_CHAR_PROP_RD | ATT_CHAR_PROP_NTF,
                                                              {0, 0},
                                                              DEF_CUST1_ADXL_VAL_UUID_128};
...
struct attm_desc_128 custs1_att_db[CUST1_IDX_NB] =
{
    ...
    // ADXL Value Characteristic Declaration
    [CUST1_IDX_ADXL_VAL_CHAR]          = {(uint8_t*)&att_decl_char, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(custs1_adxl_val_char), sizeof(custs1_adxl_val_char), (uint8_t*)&custs1_adxl_val_char},

    // ADXL Value Characteristic Value
    [CUST1_IDX_ADXL_VAL_VAL]           = {CUST1_ADXL_VAL_UUID_128, ATT_UUID_128_LEN, PERM(RD, ENABLE) | PERM(NTF, ENABLE),
                                            DEF_CUST1_ADXL_VAL_CHAR_LEN, 0, NULL},

    // ADXL Value Client Characteristic Configuration Descriptor
    [CUST1_IDX_ADXL_VAL_NTF_CFG]       = {(uint8_t*)&att_decl_cfg, ATT_UUID_16_LEN, PERM(RD, ENABLE) | PERM(WR, ENABLE),
                                            sizeof(uint16_t), 0, NULL},

    // ADXL Value Characteristic User Description
    [CUST1_IDX_ADXL_VAL_USER_DESC]     = {(uint8_t*)&att_decl_user_desc, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(CUST1_ADXL_VAL_USER_DESC) - 1, sizeof(CUST1_ADXL_VAL_USER_DESC) - 1, CUST1_ADXL_VAL_USER_DESC},

		// ECG Value Characteristic Declaration
    [CUST1_IDX_ECG_VAL_CHAR]          = {(uint8_t*)&att_decl_char, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(custs1_ecg_val_char), sizeof(custs1_ecg_val_char), (uint8_t*)&custs1_ecg_val_char},

    // ECG Value Characteristic Value
    [CUST1_IDX_ECG_VAL_VAL]           = {CUST1_ECG_VAL_UUID_128, ATT_UUID_128_LEN, PERM(RD, ENABLE) | PERM(NTF, ENABLE),
                                            DEF_CUST1_ECG_VAL_CHAR_LEN, 0, NULL},

    // ECG Value Client Characteristic Configuration Descriptor
    [CUST1_IDX_ECG_VAL_NTF_CFG]       = {(uint8_t*)&att_decl_cfg, ATT_UUID_16_LEN, PERM(RD, ENABLE) | PERM(WR, ENABLE),
                                            sizeof(uint16_t), 0, NULL},
